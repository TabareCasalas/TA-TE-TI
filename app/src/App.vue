<template>
  <div id="app">
    ...
    <div class="container">
      <div class="columns">
        ...
        <div class="column">
          <div v-if="!showStats">
          <Tateti v-on:ganadoreluno="ganadorelunofn" v-on:ganadoreldos="ganadoreldosfn" v-on:ningunogana="ningunoganafn" v-on:resetmessage="reseteandoagain"/>
          <div class="notification is-primary" v-show="this.message">
            {{this.message}}
        </div>
    </div>
          <div v-else>
  ...
  <div class="hero-body">
    <div class="container">
      ...
      <h2 class="subtitle">
        by <a href="javascript:;">Tabaré Casalas</a> & <a href="javascript:;">Christian Miguez</a>
      </h2>
    </div>
  </div>
</section>
...
export default {
  ...
  methods: {
    ...
    ganadorelunofn() {
      ...
      this.message = this.player1Nick + ' Wins.'
      var comp = this
    ...
    ganadoreldosfn() {
      ...
      this.message = this.player2Nick + ' Wins.'
      var comp = this
    ...
    ningunoganafn() {
      this.playerOne.draws++
      this.playerTwo.draws++
      this.message = 'another boring draw'
      var comp = this
      axios.put('http://localhost:4500/users/' + comp.player1Id, {draws: comp.playerOne.draws}).then((user) =>{
               console.log(user)
              }).catch(err => console.log(err))
      axios.put('http://localhost:4500/users/' + comp.player2Id, {draws: comp.playerTwo.draws}).then((user) =>{
               console.log(user)
              }).catch(err => console.log(err))
            console.log('DRAW');
    },
